...
SELECT apellido1, apellido2, p.nombre, d.nombre AS nombre_departamento FROM persona p RIGHT JOIN profesor pr ON  p.id = pr.id_profesor INNER JOIN departamento d ON pr.id_departamento = d.id ORDER BY apellido1, apellido2, p.nombre
...
SELECT a.nombre, c.anyo_inicio, c.anyo_fin FROM persona p INNER JOIN alumno_se_matricula_asignatura b ON p.id = b.id_alumno INNER JOIN curso_escolar c ON b.id_curso_escolar = c.id INNER JOIN asignatura a ON b.id_asignatura = a.id WHERE nif = "26902806M"
...
SELECT DISTINCT d.nombre FROM profesor pr INNER JOIN departamento d ON  d.id = pr.id_departamento INNER JOIN asignatura a ON pr.id_profesor = a.id_profesor INNER JOIN grado g ON a.id_grado = g.id WHERE g.nombre = 'Grado en Ingeniería Informática (Plan 2015)'
...
SELECT DISTINCT p.nombre FROM persona p INNER JOIN alumno_se_matricula_asignatura b ON p.id = b.id_alumno INNER JOIN curso_escolar c ON b.id_curso_escolar = c.id WHERE c.anyo_fin IN (2018,2019)--Tal vez deba ir solo 2018 aca, es cuestion de interpretacion de la consigna. 
...
SELECT d.nombre AS departamento_nombre, p.apellido1, p.apellido2, p.nombre AS nombre_profesor FROM profesor pr INNER JOIN persona p ON p.id = pr.id_profesor LEFT JOIN departamento d ON  d.id = pr.id_departamento ORDER BY d.nombre, p.apellido1, p.apellido2, p.nombre

--02 Devuelve un listado con los profesores que no están asociados a un departamento.

SELECT * FROM profesor pr INNER JOIN persona p ON p.id = pr.id_profesor LEFT JOIN departamento d ON  d.id = pr.id_departamento WHERE pr.id_departamento is NULL
...
SELECT DISTINCT d.nombre FROM departamento d WHERE d.nombre NOT IN (SELECT DISTINCT d.nombre FROM departamento d INNER JOIN  profesor pr ON pr.id_departamento = d.id INNER JOIN asignatura a ON a.id_profesor = pr.id_profesor INNER JOIN alumno_se_matricula_asignatura b ON a.id = b.id_asignatura INNER JOIN curso_escolar ce ON ce.id = b.id_curso_escolar);
...
SELECT COUNT(id_profesor), d.nombre AS departamento_nombre FROM profesor pr INNER JOIN departamento d ON pr.id_departamento = d.id GROUP BY d.nombre ORDER BY COUNT(id_profesor) DESC
...
SELECT d.nombre AS departamento_nombre, COUNT(id_profesor) FROM profesor pr RIGHT JOIN departamento d ON pr.id_departamento = d.id GROUP BY d.nombre ORDER BY COUNT(id_profesor) DESC
...
SELECT g.nombre AS nombre_grado, COUNT(a.id) AS numero_asignaturas FROM grado g LEFT JOIN asignatura a ON a.id_grado = g.id GROUP BY g.nombre ORDER BY COUNT(a.id) DESC
...
SELECT g.nombre AS nombre_grado, COUNT(a.id) AS numero_asignaturas FROM grado g LEFT JOIN asignatura a ON a.id_grado = g.id GROUP BY g.nombre HAVING COUNT(a.id) > 40
...
SELECT g.nombre AS nombre_grado, a.tipo AS tipo_asignatura, SUM(a.creditos) AS total_creditos FROM grado g INNER JOIN asignatura a ON g.id = a.id_grado GROUP BY a.tipo, g.nombre    
...
SELECT ce.anyo_inicio AS año_inicio_curso_escolar, COUNT(id_alumno) AS cantidad_alumnos FROM alumno_se_matricula_asignatura b INNER JOIN curso_escolar ce ON  b.id_curso_escolar = ce.id GROUP BY ce.anyo_inicio
...
SELECT p.id, p.nombre, p.apellido1, p.apellido2, COUNT(a.id) AS cantidad_asignaturas FROM profesor pr INNER JOIN persona p ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor GROUP BY p.id ORDER BY COUNT(a.id) DESC  
...
SELECT p.id, p.nombre, p.apellido1, p.apellido2 FROM profesor pr INNER JOIN persona p ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor WHERE id_departamento is NOT NULL AND a.id is NULL
